...
async function main() {
    ...
    const imgDirPath = path.join(path.resolve(__dirname, '..'), 'images', 'pioneers');
    const filesName = await fsPromises.readdir(imgDirPath, (err) => {
        if (err) {
            console.log("Import from directory failed: ", err);
        }
    })
    const imagesName = filesName.filter((fileName) => fileName.includes('.jpeg'));
    let imagesData = [];
    for await (const imageName of imagesName) {
        let imageFilePath = path.join(path.resolve(__dirname, '..'), 'images', 'pioneers', imageName);
